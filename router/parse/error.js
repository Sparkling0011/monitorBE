...
const MCommon = require('../../model/parse/common')
...
class ParseMonitor extends ParseBase {
  ...
  async processRecordAndCacheInProjectMap(record) {
    let projectId = _.get(record, ['common', 'pid'], 0)
    let visitAt = _.get(record, ['time'], moment().unix())
    let errorType = _.get(record, ['code'], '')
    let errorName = _.get(record, ['detail', 'error_no'], '')
    ...
    let md5 = _.get(record, ['md5'], _md5(JSON.stringify(record)))
      ...
      for (let [visitAtTime, monitorMap] of visitAtMap) {
        ...
        for (let monitorRecord of monitorMap) {
          ...
          if (uniqueSet.has(key) === false) {
            ...
            sqlRecord.monitor_ext_id = monitorRes._id
            monitorParams.datas = sqlRecord
            console.log("before", monitorParams)
...
async function ErrorParse(req, res, next) {
  const monitor = new ParseMonitor()
  if (!monitor.isLegalRecord) res.status(400).json({ message: "不合法的记录" })
  await monitor.processRecordAndCacheInProjectMap(req.body)
  await monitor.save2DB()
