...
// const MCityDistribution = require('~/src/model/parse/city_distribution')
const DATE_FORMAT = require('../../constants/date_format')
const DatabaseUtil = require('../../library/database')
...
async function getErrorNameDistributionInSameMonth(projectId, startAt, endAt, max = 500) {
  ...
  console.log(tableName, timeList, startAt, endAt)
  const rawDistributionList = []
  try {
    rawDistributionList = await getModel(tableName).aggregate([
      {
        $match: {
          projectId,
    ...
    console.log(rawDistributionList)
 ...
 */
async function getErrorNameDistributionByTimeWithCache(projectId, startAt, endAt) {
  let distributionList = []
  let distributionMap = {}
  for (let timeAt = startAt; timeAt <= endAt; timeAt += 86400) {
    const newDistributionList = await getErrorNameDistributionInSameMonth(projectId, moment.unix(timeAt).startOf('day').unix(), moment.unix(timeAt).endOf('day').unix())

    for (let distribution of newDistributionList) {
      let errorName = _.get(distribution, ['error_name'], '')
      let errorCount = _.get(distribution, ['error_count'], 0)
